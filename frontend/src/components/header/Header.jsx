...
const Header = () => {
  ...
  // Toggle sidebar open/close state (used on mobile)
  const toggleSidebar = () => {
    setSidebarOpen(!sidebarOpen);
  };

  // Handle window resize to show sidebar automatically on desktop sizes
  useEffect(() => {
    const handleResize = () => {
      if (window.innerWidth >= 992) {
        ...
        // For mobile, keep it closed by default
  ...
  return (
    <>
      <header id="header" className="header fixed-top d-flex align-items-center">
        <div className="d-flex align-items-center justify-content-between header-left">
          <a href="#" className="logo d-flex align-items-center">
            <img src={logo} alt="Logo" />
            <span className="d-none d-lg-flex logo-text" style={{ animation: "fadeIn 3.5s" }}>
              {role || "Loading Role..."}
            </span>
          </a>
          {/* Hamburger menu for mobile – triggers sidebar toggle */}
          <button className="hamburger-btn d-lg-none" onClick={toggleSidebar} title="Toggle Sidebar">
